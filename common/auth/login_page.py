...
from common.components.locators import LOGIN
...
class LoginPage(BasePage):
    ...
    def expect_login_error(self) -> None:
        ...
        if error.count() > 0 and error.first.is_visible():
            return
        expect(self.page).to_have_url(re.compile(".*/sign-in/?$"))
        expect(self.page.locator(LOGIN.password_input).first).to_be_visible()
